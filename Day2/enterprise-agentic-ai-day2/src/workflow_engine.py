...
class WorkflowContext:
    ...
    def transition(self, new_state: WorkflowState, detail: str):
        self.history.append({
...
class SupplierOnboardingWorkflow:
    ...
    def advance(self, detail: str = ""):
        ...
        self.context.transition(
            next_state,
            detail or f"Moved from {current.value} to {next_state.value}",
        )
        return self.context
    ...
    def route_after_risk(self, risk_score: int, threshold: int):
        if risk_score >= threshold:
            self.context.transition(
                ...
                f"Risk score {risk_score} exceeds threshold {threshold}",
            )
        else:
            self.context.transition(
    ...
    def complete(self, approved: bool, detail: str):
