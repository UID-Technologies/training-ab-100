...
def generate_observability_report(module_filter=None):
    ...
    if module_filter:
        ...
        metrics = [row for row in metrics if row.get("module") == module_filter]

    total_steps = len(traces)
    failed_steps = len([row for row in traces if row.get("status") == "FAILED"])
    success_rate = round(((total_steps - failed_steps) / total_steps) * 100, 2) if total_steps else 100.0
    ...
    return {
        ...
        "total_trace_events": total_steps,
        "failed_events": failed_steps,
